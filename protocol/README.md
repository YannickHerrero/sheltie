...
Terminal bytes are base64-encoded so WebSocket JSON frames remain inspectable and deterministic. A `full` terminal frame replaces local emulator state; incremental frames are accepted only in sequence. Canonical scrollback is requested separately as a bounded, read-only `terminal.history` snapshot because live viewport frames do not contain terminal history. Additive capability-gated messages cover revision-checked workspace todo documents and per-device notification configuration.
