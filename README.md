...
- Explicit-button, read-only terminal history backed by Herdr's recent scrollback
- Touch composers, repeatable Page Up/Down keys, sticky modifiers, context menus, and keyboard commands
...
Configure the production variables in `bridge/.env.example`, run the bridge with a local service manager, and expose it under a dedicated path. On macOS, [`bridge/citadel.service.json`](bridge/citadel.service.json) and [`bridge/herdr.citadel.service.json`](bridge/herdr.citadel.service.json) provide reusable Citadel contracts for the bridge and its persistent Herdr server:
